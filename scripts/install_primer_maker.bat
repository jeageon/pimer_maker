...
if errorlevel 1 (
    echo [WARN] 관리자 권한이 없어 기본 경로에 설치할 수 없습니다.
    set "INSTALL_DIR=%LOCALAPPDATA%\Programs\%APP_NAME%"
    echo [INFO] 사용자 폴더로 전환하여 설치합니다: %INSTALL_DIR%
    if not exist "%INSTALL_DIR%" mkdir "%INSTALL_DIR%" >nul 2>&1
)

if errorlevel 1 (
    echo [ERROR] 설치 폴더를 준비할 수 없습니다.
    pause
    exit /b 1
)
...
echo echo Primer Maker가 삭제되었습니다.
echo pause
) > "%UNINSTALLER%"
...
if not "%INSTALL_ROOT%"=="%INSTALL_DIR%" (
    echo [INFO] 사용자 폴더로 우회 설치되었습니다.
)
echo 바탕화면 및 시작메뉴 바로가기가 생성되었습니다.
echo 실행: "%%USERPROFILE%%\Desktop\%APP_NAME%.lnk"
echo 제거: 설치 폴더의 uninstall_primer_maker.bat 실행
pause
exit /b 0
